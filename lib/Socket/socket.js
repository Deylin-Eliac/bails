...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        const pairingCode = pairKey ? pairKey.toUpperCase() : 'KIRITO16';
        authState.creds.pairingCode = pairingCode;
        authState.creds.me = {
            id: (0, WABinary_1.jidEncode)(phoneNumber, 's.whatsapp.net'),
            name: '~'
        };
        // NOTE: authState.creds.pairingEphemeralKeyPair must be generated before this call.
        // It's usually done during authState initialization in a higher-level function.
    ...
    async function generatePairingKey() {
        if (!authState.creds.pairingEphemeralKeyPair) {
            throw new Error('pairingEphemeralKeyPair is not initialized on creds');
        }
